...
$(function () {
  ...
  $maps.each(function () {
    ...
    editors[id].onClickArea(function (area_id, area) {
      ...
      $modal
        .find(".modal-content")
        .load(
          `/admin/navigation_maps/blueprints/${id}/areas/${rel}`,
          function () {
            var $input1 = $modal.find('input[name="blueprint_area[area_id]"]');
            var $input2 = $modal.find(
              'input[name="blueprint_area[area_type]"]'
            );
  ...
  document.body.addEventListener("ajax:success", function (responseText) {
    ...
    editors[blueprint_id].setLayerProperties(
      editors[blueprint_id].map._layers[area_id],
      area
    );
  ...
  var needsReload = function () {
    ...
    $form
      .find("input[type=file],input[tabs_id=blueprints___title]")
      .each(function () {
        if ($(this).val()) {
          reload = true;
          return false;
  ...
  $form.ajaxForm({
    ...
    beforeSerialize: function () {
      Object.keys(editors).forEach(function (key) {
        ...
        $(`#blueprints_${editor.id}_blueprint`).val(
          JSON.stringify(editor.getBlueprint())
        );
      });
    },
    beforeSend: function () {
